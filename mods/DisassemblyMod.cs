...
public static class DisassemblyMod
{
    ...
    /// <summary>Items a recipe was NOT generated for, with the reason. See the two checks below.</summary>
    private static readonly List<string> refused = new List<string>();

    /// <summary>What was refused on the last data load, and why. Printed by dataexport --disassembly.</summary>
    ...
    private static ProcessType Build(EntityType item, ProcessType production, ProcessType template)
    {
        ...
        // The two checks that keep this honest, added after reading the studio's own source.
        if (!ReturnsNoMoreThanWasConsumed(item, production, batch, recovered)
            || !AgreesWithDeclaredParts(item, recovered))
        {
            return null;
        }
    ...
    /// <summary>
    /// Whether the recovery gives back no more of anything than the production recipe consumed of
    /// it - counting a degraded material against the material it came from.
    ///
    /// <see cref="Build"/> constructs the outputs from those very inputs, so this can only fail if
    /// somebody changes that code or a table underneath it. That is exactly why it is here: the
    /// studio's validator enforced a rule of its own on salvage recipes
    /// (<c>ProcessType.PostDataCompleteValidate</c> - "validate that the out type/amount exists in
    /// the parts list of the one input entity"), and this mod's items have no parts list for it to
    /// check against, so the port had to relax that rule to load at all. Relaxing a rule without
    /// putting the equivalent one in its place is how a duplicator gets shipped, and a recipe that
    /// hands back more than went in is precisely what their rule was protecting the game from.
    /// </summary>
    private static bool ReturnsNoMoreThanWasConsumed(EntityType item, ProcessType production,
                                                     int batch, List<Output> recovered)
    {
        foreach (Output output in recovered)
        {
            int consumed = 0;
            foreach (Input input in production.Inputs)
            {
                ...
                // The material itself, or the material this output is the degraded form of.
                DegradesTo.TryGetValue(input.Entity, out string lesser);
                if (input.Entity == output.EntityTypeToCreate || lesser == output.EntityTypeToCreate)
                {
                    consumed += input.Amount?.NoOfItems ?? 1;
                }
            }

            int given = output.Amount?.NoOfItems ?? 1;
            if (consumed < 1 || given > consumed / batch)
            {
                refused.Add(item.KeyName + " - would return " + given + " " + output.EntityTypeToCreate
                            + " where making one consumed " + consumed + "/" + batch);
                return false;
            }
        }
        return true;
    ...
    /// <summary>
    /// Whether the recovery satisfies the studio's rule for items that DO declare what they are
    /// made of - and refuses to generate a recipe when it cannot.
    ///
    /// THE RULE, from their source. <c>NonLivingType.Parts</c> is documented as "the number of
    /// parts that the item is composed of - production may also consume other materials though",
    /// and <c>ProcessType.PostDataCompleteValidate</c> requires every non-waste output of a salvage
    /// process to be one of those declared parts, in no greater amount. Parts and consumed
    /// materials are different things in their model, and a salvage recipe is about the parts.
    ///
    /// This mod works the other way round - it gives back the materials - which is a departure,
    /// and the port carries a null guard so that an item with NO parts list is left out of their
    /// rule rather than crashing in it. An item that DOES declare its parts is a different case:
    /// their rule applies, their validator will check it, and a generated recipe that disagrees
    /// would take the game down on the loading screen with a validation error. Six stock items
    /// declare parts without a disassembly (arrows, the wheel, the spike trap, the power unit) and
    /// none of them is in a category this mod considers - but a scenario overlay or another mod
    /// can put one there, and then this is the difference between a skipped recipe and a crash.
    ///
    /// PartKeys rather than Parts: the resolved dictionary is filled later, in
    /// <c>NonLivingType.PostLoadContentInitialize</c>, and this runs while the tables are still
    /// being built.
    /// </summary>
    private static bool AgreesWithDeclaredParts(EntityType item, List<Output> recovered)
    {
        SerializableDictionary<string, int> parts = item.NonLivingType.PartKeys;
        if (parts == null)
        {
            return true;
        }
        ...
        foreach (Output output in recovered)
        {
            if (!parts.TryGetValue(output.EntityTypeToCreate, out int declared)
                || (output.Amount?.NoOfItems ?? 1) > declared)
            {
                refused.Add(item.KeyName + " - declares its parts, and " + output.EntityTypeToCreate
                            + " is not one of them (the studio's salvage rule wins)");
                return false;
            }
        }
        return true;
