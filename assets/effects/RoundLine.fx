...
VS_OUTPUT MyVS(VS_INPUT In)
{
	...
	float y0 = instanceData[In.instanceIndex].y; // always zero?!?!?!
	float rho = instanceData[In.instanceIndex].z;
	float theta = instanceData[In.instanceIndex].w;
	
	//y0 = In.instanceIndex * 20; //50;

	/*float x0 = instanceData[3].x;
	float y0 = instanceData[3].y;
	float rho = instanceData[3].z;
	float theta = instanceData[3].w;
	*/
	...
	// ORiginal:
	Out.posModelSpace.xy = pos.xy; // not used in standard effect
...
float4 MyPSStandard(VS_OUTPUT input) : COLOR0
{
	float4 finalColor;
...
float4 MyPSAnimatedLinear(VS_OUTPUT input) : COLOR0
{
	float4 finalColor;
	float modulation = sin((input.posModelSpace.x * 0.1 + time * 0.05) * 80 * 3.14159) * 0.5 + 0.5;
...
float4 MyPSAnimatedRadial(VS_OUTPUT input) : COLOR0
{
	float4 finalColor;
...
float4 MyPSModern(VS_OUTPUT input) : COLOR0
{
	float4 finalColor;
	finalColor.rgb = lineColor.rgb;
...
float4 MyPSTubular(VS_OUTPUT input) : COLOR0
{
	float4 finalColor = lineColor;
	finalColor.a *= input.polar.x;
	finalColor.a = finalColor.a * BlurEdge(input.polar.x);
	return finalColor;
}

//float4 MyPSGlow(float3 polar : TEXCOORD0) : COLOR0
float4 MyPSGlow(VS_OUTPUT input) : COLOR0
{
	float4 finalColor = lineColor;
...
technique Standard
{
	pass P0
	{
		...
		BlendOp = Add;
		vertexShader = compile vs_3_0 /*vs_1_1*/ MyVS();
		pixelShader = compile ps_3_0 /*ps_2_0*/ MyPSStandard();
	}
}
...
technique NoBlur
{
	pass P0
	{
		...
		BlendOp = Add;
		vertexShader = compile vs_3_0 /*vs_1_1*/ MyVS();
		pixelShader = compile ps_3_0 /*ps_2_0*/ MyPSNoBlur();
	}
}
...
technique AnimatedLinear
{
	pass P0
	{
		...
		BlendOp = Add;
		vertexShader = compile vs_3_0 /*vs_1_1*/ MyVS();
		pixelShader = compile ps_3_0 /*ps_2_0*/ MyPSAnimatedLinear();
	}
}
...
technique AnimatedRadial
{
	pass P0
	{
		...
		BlendOp = Add;
		vertexShader = compile vs_3_0 /*vs_1_1*/ MyVS();
		pixelShader = compile ps_3_0 /*ps_2_0*/ MyPSAnimatedRadial();
	}
}
...
technique Modern
{
	pass P0
	{
		...
		BlendOp = Add;
		vertexShader = compile vs_3_0 /*vs_1_1*/ MyVS();
		pixelShader = compile ps_3_0 /*ps_2_0*/ MyPSModern();
	}
}
...
technique Tubular
{
	pass P0
	{
		...
		BlendOp = Add;
		vertexShader = compile vs_3_0 /*vs_1_1*/ MyVS();
		pixelShader = compile ps_3_0 /*ps_2_0*/ MyPSTubular();
	}
}
...
technique Glow
{
	pass P0
	{
		...
		BlendOp = Add;
		vertexShader = compile vs_3_0 /*vs_1_1*/ MyVS();
		pixelShader = compile ps_3_0 /*ps_2_0*/ MyPSGlow();
	}
}
