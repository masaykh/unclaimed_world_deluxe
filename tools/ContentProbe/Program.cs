...
{
    /// <summary>Set by --effect-params; see the flag parsing in Main.</summary>
    internal static bool EffectParamNames;
    ...
    private static int Main(string[] args)
    {
        ...
        // --effect-params: list every parameter NAME each effect exposes. The game reaches
        // parameters by name and does not check, so a backend whose compiler drops one is a
        // NullReferenceException the first frame that draw runs - and for the FNA target there is
        // no offline equivalent of `mgfxtranscode compare-params`, because those effects are D3D9
        // fx_2_0 blobs rather than MGFX. Reading them back off a live device is the only way.
        EffectParamNames = args.Contains("--effect-params");
    ...
    private sealed class Probe : Game
    {
        ...
        /// <summary>
        /// What the GPU actually samples out of a texture, as opposed to what loaded.
        ///
        /// WHY THIS IS NOT PARANOIA. Every sprite in the world is drawn as
        /// <c>lerp(tex, tex * color, lightingEffect)</c> with <c>tex.a</c> for alpha, so a texture
        /// whose RGB reads back as zero while its ALPHA is intact produces exactly what DesktopGL
        /// was producing: every tree, building and character a correctly-shaped black silhouette,
        /// with no error anywhere and the shader itself provably correct on a bench.
        /// "It loaded" cannot distinguish that from a working texture; only reading pixels can.
        ///
        /// Drawn through SpriteBatch into a render target rather than GetData'd directly, because
        ...
        private string DescribeTextureContent(Texture2D texture)
        {
            ...
            try
            {
                using var target = new RenderTarget2D(GraphicsDevice, Size, Size);
                GraphicsDevice.SetRenderTarget(target);
                GraphicsDevice.Clear(Color.Transparent);
                using (var batch = new SpriteBatch(GraphicsDevice))
                {
                    batch.Begin(SpriteSortMode.Immediate, BlendState.Opaque);
                    batch.Draw(texture, new Rectangle(0, 0, Size, Size), Color.White);
                    batch.End();
                }
                ...
                int opaqueButBlack = 0, opaque = 0;
                foreach (Color p in pixels)
                {
                    r += p.R; g += p.G; b += p.B; a += p.A;
                    if (p.A <= 8) continue;
                    opaque++;
                    if (p.R == 0 && p.G == 0 && p.B == 0) opaqueButBlack++;
                }

                int n = pixels.Length;
                string verdict = opaque > 0 && opaqueButBlack == opaque
                    ? "  <-- EVERY visible pixel is pure black; the RGB channels did not survive decoding"
                    : "";
                return $" mean rgba=({r / n},{g / n},{b / n},{a / n}), " +
                       $"{opaqueButBlack}/{opaque} visible pixel(s) pure black{verdict}";
            }
            catch (Exception ex)
            {
                return $" (could not sample: {ex.GetType().Name}: {ex.Message})";
            }
        }

        /// <summary>
        /// The scalar/vector parameters that arrive with a non-zero INITIAL VALUE.
        ///
        /// Several uniforms in this game are declared with an HLSL initialiser and never assigned
        /// from code - Billboard's LightColor, Vehicle's Alpha - so the initialiser is the value
        /// for the life of the run. mgfxc's OpenGL path writes zero for every default where its
        /// DirectX path writes the initialiser (PORT DEVIATION 21), and the result is not an
        /// error: the effect loads, the shader compiles, the draw happens, and the world renders
        /// as black silhouettes with every character transparent. Printing what actually reached
        /// the loaded effect is the only check that sees it from the runtime's side rather than
        /// from the container's.
        /// </summary>
        private static string DescribeEffectInitialValues(Effect fx)
        {
            var initialised = new List<string>();
            foreach (EffectParameter p in fx.Parameters)
            {
                if (p.ParameterClass != EffectParameterClass.Scalar &&
                    p.ParameterClass != EffectParameterClass.Vector)
                {
                    continue;
                }
                if (p.Elements.Count > 0 || p.ParameterType != EffectParameterType.Single)
                {
                    continue;
                }

                float[] values;
                // XNA's signature takes the element count; MonoGame added a parameterless
                // overload and FNA, reimplementing XNA, does not have it.
                try
                {
#if UW_FNA
                    values = p.GetValueSingleArray(Math.Max(p.RowCount, 1) * Math.Max(p.ColumnCount, 1));
#else
                    values = p.GetValueSingleArray();
#endif
                }
                catch { continue; }
                if (values == null || values.All(v => v == 0f))
                {
                    continue;
                }
                initialised.Add(p.Name + "=" +
                    string.Join("/", values.Select(v => v.ToString("0.###", System.Globalization.CultureInfo.InvariantCulture))));
            }

            string names = EffectParamNames
                ? Environment.NewLine + "            params: " +
                  string.Join(", ", fx.Parameters.Select(p => p.Name).OrderBy(n => n, StringComparer.Ordinal))
                : "";

            // ARRAY parameters, with their declared shape. FNA's SetValue(Matrix[]) only knows
            // 4x4, 3x3, 4x3 and 3x4 and writes NOTHING for any other shape - so a bone palette
            // the compiler narrowed to, say, 4x2 is silently left as zeros, every vertex lands at
            // w=0, and the mesh explodes through the perspective divide. The shape is the thing
            // that decides it, and nothing else reports it.
            var arrays = fx.Parameters
                .Where(p => p.Elements.Count > 0)
                .Select(p => $"{p.Name}[{p.Elements.Count}] {p.ParameterClass}/{p.ParameterType} " +
                             $"{p.RowCount}x{p.ColumnCount}")
                .ToList();
            if (EffectParamNames && arrays.Count > 0)
            {
                names += Environment.NewLine + "            arrays: " + string.Join(", ", arrays);
            }

            return names + (initialised.Count == 0
                ? "  <-- NO parameter carries a non-zero initial value"
                : Environment.NewLine + "            initial values: " + string.Join(", ", initialised));
        }

        /// <summary>
        /// What the model carries in Tag - which for this game is the SKINNING INFORMATION.
        ///
        /// ModelAnimator reads it as SkinInfoCollection[], one per mesh, and sizes each bone
        /// palette from it: `palette[i] = new Matrix[skinInfo[i].Count]`. A collection that
        /// arrives EMPTY produces a zero-length palette, so CopyAbsoluteTransforms fills nothing,
        /// the shader keeps the zero matrices it started with, every vertex lands at w=0 and the
        /// mesh explodes through the perspective divide. That failure is indistinguishable from a
        /// dozen others by eye, and nothing else reports the count.
        /// </summary>
        private static string DescribeModelTag(Model m)
        {
            if (m.Tag == null)
            {
                return "  <-- Tag is NULL; ModelAnimator needs skinning info here";
            }
            // ModelAnimator's own shape: Dictionary<string, object> with "SkinInfo" holding a
            // SkinInfoCollection[], one per mesh.
            if (m.Tag is System.Collections.Generic.Dictionary<string, object> tag)
            {
                ...
                if (tag.TryGetValue("SkinInfo", out object value) && value is SkinInfoCollection[] skins)
                {
                    return $" tag=[{keys}] SkinInfo[{skins.Length}] bones-per-mesh=" +
                           ...
                           (skins.All(s => s == null || s.Count == 0)
                               ? "  <-- EMPTY: the bone palette is sized from this, so it stays all-zero"
                               : "");
                }
        ...
        private string Describe(object o) => o switch
        {
            ...
            ExtendedSpriteSheet e =>
                ...
                DescribeTextureContent(e.Texture),
            SpriteSheet s => $"SpriteSheet, {s.spriteRectangles?.Count ?? -1} rects" +
                DescribeTextureContent(s.Texture),
            Model m => $"Model, {m.Meshes.Count} meshes, {m.Bones.Count} bones" +
