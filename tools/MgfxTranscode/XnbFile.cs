...
internal sealed class XnbFile
{
    ...
    /// <summary>
    /// The container's contents with the header removed and any LZ4 decompression already done -
    /// the reader table, then the object graph. Exposed for XnbExtract, which reads asset types
    /// this tool does not (textures, models), from the same parsed container.
    /// </summary>
    public byte[] Payload => _payload;

    /// <summary>Offset into <see cref="Payload"/> where the primary object's own data starts.</summary>
    public int PrimaryDataOffset => _primaryDataOffset;
    ...
    /// <summary>
    /// Writes the file back unchanged except that it is UNCOMPRESSED.
    ///
    /// Why this is needed at all: MonoGame compresses XNBs with LZ4, which XNA never had - XNA
    /// used LZX, flag 0x80, and MonoGame gave its own scheme flag 0x40. FNA implements XNA, so it
    /// ships an LZX decoder and tests only <c>(flags &amp; 0x80) != 0</c>. Handed a MonoGame
    /// container it therefore concludes the file is NOT compressed, reads the LZ4 payload as if it
    /// were the object graph, and fails with an EMPTY type-reader name:
    ///
    ///     Could not find ContentTypeReader Type ... :  ()
    ///
    /// which reads like a missing assembly and is really a container it cannot decompress.
    ///
    /// Decompressing is enough, and is the whole fix: an uncompressed XNB is valid for XNA,
    /// MonoGame and FNA alike. It costs disk space and saves load time.
    /// </summary>
    /// <summary>
    /// Writes the file back with a replacement payload - the whole object graph after the
    /// 10-byte header - always uncompressed. The size field is recomputed, which is the only
    /// thing in an XNB that depends on the payload length.
    /// </summary>
    public void WriteWithPayload(string path, byte[] payload)
    {
        byte flags = (byte)(Flags & ~(FlagCompressedLz4 | FlagCompressedLzx));
    ...
    public void WriteDecompressed(string path)
    {
        byte flags = (byte)(Flags & ~(FlagCompressedLz4 | FlagCompressedLzx));
        ...
        outStream.WriteByte(Version);
        outStream.WriteByte(flags);
        outStream.Write(BitConverter.GetBytes(10 + _payload.Length), 0, 4);
