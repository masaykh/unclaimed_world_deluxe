...
internal static class SkinnedVertexCompat
{
    ...
    {
#if UW_FNA
        // NOT APPLIED ON FNA, and it could not be: XNA 4.0's ModelMeshPart.VertexBuffer is
        // read-only, so the replacement this class performs has nowhere to land. MonoGame added
        // the setter.
        //
        // That is not merely an obstacle - it is a hint. This whole class works around MonoGame
        // deciding vertex-attribute normalization from an element's USAGE rather than its FORMAT,
        // which is a documented divergence from XNA (MonoGame's own source carries the TODO
        // asking which one XNA did). FNA reimplements XNA rather than extending it, so the
        // expectation is that it normalizes a Color-format element correctly and needs no help.
        //
        // EXPECTATION, NOT MEASUREMENT. If skinned models come out scaled by about 255 on FNA -
