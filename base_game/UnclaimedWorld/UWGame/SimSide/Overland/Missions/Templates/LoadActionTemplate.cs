...
public class LoadActionTemplate : MissionActionTemplate
{
	...
	{
		Orders = new SerializableDictionary<string, List<long>>(orders.ToDictionary((KeyValuePair<EntityType, List<EntityID>> k) => k.Key.KeyName, (KeyValuePair<EntityType, List<EntityID>> k) => k.Value.Select((EntityID e) => (long)e).ToList()));
	}
